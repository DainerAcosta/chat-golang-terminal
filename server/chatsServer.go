...
var (
	inclomingClients = make(chan Client)
...
func HandleConnection(conn net.Conn) {
	...
	message <- fmt.Sprintf("Welcome to the serve, your name %s\n", clientName)
	messages <- fmt.Sprintf("New client is here, name %s\n", clientName)
	inclomingClients <- message
...
func Broadcast() {
	...
	for {
		...
		case newClient := <-inclomingClients:
			clients[newClient] = true
		case leavingClients := <-leavingClients:
			delete(clients, leavingClients)
